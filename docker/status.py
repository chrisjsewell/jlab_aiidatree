def verdi_status(with_rmq=False, with_daemon=False):
    ...
    try:
        ...
        print_status(
            ServiceStatus.ERROR, "repository", "Error with repo folder", exception=exc
        )
    ...
    try:
        ...
        print_status(
            ServiceStatus.UP, "postgres", "Connected as {}@{}:{}".format(*database_data)
        )
    ...
    try:
        ...
        print_status(
            ServiceStatus.UP, "rabbitmq", "Connected to {}".format(get_rmq_url())
        )
    ...
    try:
        ...
        print_status(
            ServiceStatus.ERROR, "daemon", "Error getting daemon status", exception=exc
        )
        exit_code = ExitCode.CRITICAL
